...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string; highlightId: string }> }) {
  try {
    ...
    const updateResponse = await fetch(`https://api.nobelyayin.com/books/${id}/highlights/${highlightId}`, {
      method: "PUT",
      headers: {
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string; highlightId: string }> }) {
  try {
    const loginData = {
      email: "api@test.com",
      password: "test123",
    ...
    const deleteResponse = await fetch(`https://api.nobelyayin.com/books/${id}/highlights/${highlightId}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
